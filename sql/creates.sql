...
ALTER TABLE `track` ADD CONSTRAINT chk_track_duration CHECK (duration_seconds > 0);
ALTER TABLE `history` ADD CONSTRAINT chk_history_duration CHECK (duration_listened_seconds >= 0);
ALTER TABLE `user` ADD CONSTRAINT chk_user_email CHECK (email LIKE '%@%.%');
ALTER TABLE `playlist_track` ADD CONSTRAINT chk_position CHECK (position > 0);
